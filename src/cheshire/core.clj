...
(s/fdef
  create-generator
  :args
  (s/cat :writer (partial instance? java.io.StringWriter))
  :ret
  (partial
    instance?
    com.fasterxml.jackson.core.json.WriterBasedJsonGenerator))
(s/def :clojure.core.typed.unqualified-keys/key-fn ifn?)
(s/def :clojure.core.typed.unqualified-keys/indentation int?)
(s/def :clojure.core.typed.unqualified-keys/escape-non-ascii boolean?)
(s/def :clojure.core.typed.unqualified-keys/before-array-values string?)
(s/def
  :clojure.core.typed.unqualified-keys/pretty
  (s/or
    :nil?
    nil?
    :boolean?
    ...
    :empty-coll
    (s/and coll? empty?)))
(s/def :clojure.core.typed.unqualified-keys/indent-arrays? false?)
(s/def
  :clojure.core.typed.unqualified-keys/object-field-value-separator
  string?)
(s/def :clojure.core.typed.unqualified-keys/date-format string?)
(s/def :clojure.core.typed.unqualified-keys/indent-objects? false?)
(s/def :clojure.core.typed.unqualified-keys/after-array-values string?)
(s/def
  ::AfterArrayValuesBeforeArrayValuesDateFormatMap
  (s/keys
    :opt-un
    [:clojure.core.typed.unqualified-keys/after-array-values
     :clojure.core.typed.unqualified-keys/before-array-values
     :clojure.core.typed.unqualified-keys/date-format
     :clojure.core.typed.unqualified-keys/escape-non-ascii
     :clojure.core.typed.unqualified-keys/indent-arrays?
     :clojure.core.typed.unqualified-keys/indent-objects?
     :clojure.core.typed.unqualified-keys/indentation
     :clojure.core.typed.unqualified-keys/key-fn
     :clojure.core.typed.unqualified-keys/object-field-value-separator
     :clojure.core.typed.unqualified-keys/pretty]))
(s/fdef
  create-pretty-printer
  :args
  (s/cat :options ::AfterArrayValuesBeforeArrayValuesDateFormatMap)
  :ret
  (partial instance? cheshire.prettyprint.CustomPrettyPrinter))
(s/fdef
  decode
  :args
  (s/alt
    ...
    (s/cat
      ...
      (s/nilable string?)
      :key-fn
      (s/or :nil? nil? :boolean? boolean? :ifn? ifn?))
    :3-args
    (s/cat :string string? :key-fn false? :array-coerce-fn ifn?))
  :ret
  any?)
(s/fdef
  decode-strict
  ...
  (s/map-of any? int?))
(s/def
  :clojure.core.typed.unqualified-keys/foo
  (s/or
    :kw
    #{:bar}
    :java.sql.Timestamp
    (partial instance? java.sql.Timestamp)
    :int?
    int?
    :string?
    string?
    :java.util.Date
    (partial instance? java.util.Date)))
(s/def :clojure.core.typed.unqualified-keys/ulu string?)
(s/def
  ::UluMap
  (s/keys :req-un [:clojure.core.typed.unqualified-keys/ulu]))
(s/def :clojure.core.typed.unqualified-keys/moot string?)
(s/def
  ::MootMap
  (s/keys :req-un [:clojure.core.typed.unqualified-keys/moot]))
(s/def
  :clojure.core.typed.unqualified-keys/baz
  (s/or :int? int? :tuple (s/tuple ::UluMap ::MootMap int?)))
(s/def
  ::BazMap
  (s/keys :req-un [:clojure.core.typed.unqualified-keys/baz]))
(s/def
  ::alias__9839
  (s/or
    :int?
    int?
    :tuple
    (s/tuple ::alias__9839 #{:quux} (s/tuple int? int? int?))
    :BazMap
    ::BazMap
    :alias__9839
    ::alias__9839))
(s/def :clojure.core.typed.unqualified-keys/bar (s/and ::alias__9839))
(s/def :clojure.core.typed.unqualified-keys/quux #{:blub})
(s/def
  ::FooBarQuuxMap
  (s/keys
    :req-un
    [:clojure.core.typed.unqualified-keys/foo]
    :opt-un
    [:clojure.core.typed.unqualified-keys/bar
     :clojure.core.typed.unqualified-keys/quux]))
(s/def
  ::alias__9890
  (s/or
    :tuple
    (s/tuple ::alias__9890 ::alias__9890)
    :FooBarQuuxMap
    ::FooBarQuuxMap))
(s/fdef
  encode
  ...
  (s/alt
    ...
    (s/cat
      ...
      (s/or
        ...
        number?
        :java.util.HashMap
        (partial instance? java.util.HashMap)
        :java.net.URL
        (partial instance? java.net.URL)
        :java.util.ArrayList
        (partial instance? java.util.ArrayList)
        :cheshire.test.core.TestR
        (partial instance? cheshire.test.core.TestR)
        :cheshire.test.core.CTestR
        (partial instance? cheshire.test.core.CTestR)
        :coll
        (s/coll-of any?)
        :map
        (s/map-of any? any?)))
...
(s/fdef
  ...
  (s/alt
    :1-arg
    (s/cat :obj (s/map-of any? any?))
    :2-args
    (s/cat :obj (s/map-of any? any?) :opt-map nil?))
  :ret
...
(s/fdef
  ...
  (s/alt
    ...
    (s/cat
      ...
      (s/map-of any? string?)
      :writer
      (partial instance? java.io.BufferedWriter))
    :3-args
    (s/cat
      :obj
      (s/map-of any? string?)
  ...
  (s/alt
    ...
    (s/cat
      ...
      (s/or
        ...
        (s/map-of any? any?))
      :opt-map
      (s/nilable ::AfterArrayValuesBeforeArrayValuesDateFormatMap)))
  :ret
  string?)
(s/fdef
  parse-smile
  ...
  (s/alt
    :1-arg
    (s/cat :bytes (partial instance? (Class/forName "[B")))
    :3-args
    (s/cat
      :bytes
      (partial instance? (Class/forName "[B"))
  ...
  :ret
  (s/map-of any? any?))
(s/def :clojure.core.typed.unqualified-keys/foo baz string?)
(s/def
  ::Foo bazMap
  (s/keys :req-un [:clojure.core.typed.unqualified-keys/foo baz]))
(s/fdef
  parse-stream
  :args
  (s/alt
    :1-arg
    (s/cat :rdr (partial instance? java.io.BufferedReader))
    :2-args
    (s/cat
      :rdr
    ...
    (s/cat
      ...
      (s/or
        ...
        (partial instance? java.io.BufferedReader))
      :key-fn
      (s/nilable boolean?)
      :array-coerce-fn
      nil?))
  :ret
  (s/or :map (s/map-of any? string?) :Foo bazMap ::Foo bazMap))
(s/fdef
  parse-string
  :args
  (s/alt
...
(s/fdef
  ...
  (s/alt
    ...
    (s/cat :string string?)
    :3-args
    (s/cat :string string? :key-fn nil? :array-coerce-fn nil?))
  :ret
  (s/map-of any? int?))
(s/fdef
  parsed-seq
  ...
  (s/alt
    :1-arg
    (s/cat :reader (partial instance? java.io.BufferedReader))
    :3-args
    (s/cat
      :reader
      (partial instance? java.io.BufferedReader)
  ...
  (s/coll-of
    (s/or
      ...
      int?
      :map
      (s/map-of any? (s/or :string? string? :int? int?)))))
(s/fdef
  parsed-seq*
  :args
  (s/cat
...
(s/fdef
  ...
  (s/alt
    ...
    (s/cat
      :obj
      (s/or :int? int? :string? string? :tuple (s/tuple string?)))
    :2-args
    (s/cat
      :obj
      any?
